...
path_dir="$(pwd)/results/rankers/${type_graph}_${nsrc}src/N_${N}"
...
init_name="${n_test_rnd}rnd_tau${tau}_d${delta}_"
...
SIB_PARS="--p_source 1e-4 --sib_maxit 200 --p_sus 0.6"
...
do
    echo $seed
    python $SCRIPT $GEN_GRAPH --seed $seed $EXTRA_FLAGS $SPARSE_OBS --start_conf $st_conf --num_conf $num_conf $EXPER $EXTRA_GEN
done